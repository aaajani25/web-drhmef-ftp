...
<div class="panel-grid" id="pgx-7-1" style="margin-top:2px;">
  ...
  <tr>
    <td colspan="2"><input type="text" name="tel" class="champ_de_saisie" required readonly id="tel" value="<?php echo $this->session->userdata('cellulaire'); ?>"></td>
  </tr>
  <tr><td colspan="2">&nbsp;</td></tr>
  <tr>
    <td>
  ...
  <tr>
    ...
    <tr>
      <td>
          <select name="emploid" class="champ_de_saisie" disabled  id="emploi">
          <option >--choix--</option>
          <!--<option selected="selected">--test--</option>-->
          <?php foreach ($emplois as $emploi){ ?>
          <option value="<?php echo $emploi['EMP_CODE']  ?>"<?php if($emploi['EMP_LIBELLE']==$this->session->userdata('emploi')) { $emp=$emploi['EMP_CODE'];echo 'selected="selected"';} else {echo '';} ?>><?php echo $emploi['EMP_LIBELLE'] ;  ?></option>
          <?php } ?>
        </select>
          <input type="hidden" name="emploi" value="<?php echo $emp; ?>">
      </td>
    </tr>
      </table>
  </div>
...
<div class="panel-grid-cell" id="pgcx-7-1-2">
  ...
  <tr>
    <td>
      <select name="acte" class="champ_de_saisie" id="acte">
