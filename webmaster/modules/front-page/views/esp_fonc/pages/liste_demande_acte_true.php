...
<table width="100%" border="0" align="center" cellpadding="7" cellspacing="3" id="tabb">
    ...
    foreach($liste as $tab_liste){$d=''; ?>
        <tr align="center" class="row-content">          
          ...
          <td><a href="?da=5&id=<?php echo $tab_liste['NUM_RECU'] ?>">
            <?php 
      if($tab_liste['ID_ACTE']!='A0041'){
      ?>
             Imprimer le reçu
            <?php } else { 
    if(empty($tab_liste['DATE_PAIE'])){
     echo 'A Payer';
    }
    else{
     echo "-"; 
          ...
          <td><?php if(!empty($tab_liste['DATE_PAIE'])){echo 'DEJA PAYE';} if($tab_liste['ID_ACTE']=='A0041' and empty($tab_liste['DATE_PAIE'])){?>
            ...
            <?php } else if($tab_liste['ID_ACTE']=='A0097'){ ?>
            <a href="javascript:void(0);" onClick="del_demande_acte('<?php echo $tab_liste['NUM_RECU'] ?>', '<?php echo $this->session->userdata('matricule')?>')">SUPPRIMER</a>
            <?php }?>
            </td>
