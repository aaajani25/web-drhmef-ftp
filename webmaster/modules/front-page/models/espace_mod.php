...
//            $nb = $this->getSQL('sf', $verif_del_mad, 'row');
    ...
    public function list_emploi() {
        $emploi = "SELECT * FROM emploi where EMP_LIBELLE NOT IN('15','16','20','98','99','9999','*UL*','97B','99B','99BR','WWWW','11','12','13','14') order by EMP_LIBELLE";
        return $this->getSQL('db', $emploi, '');
    }

    public function get_emploi_lib($emp_cod)
    {
        $emploi = "SELECT EMP_LIBELLE FROM emploi where id ='".$emp_cod."'";
        $lib=$this->getSQL('db', $emploi, '');
        return $lib[0]['EMP_LIBELLE'];
        
    }
    
    public function list_ministere() {
        $ministere = "SELECT * FROM structure WHERE code <> 'S0001' order by libelle";
        return $this->getSQL('db', $ministere, '');
    }

    public function get_ministere_lib($code) {
        $ministere = "SELECT * FROM structure WHERE  code ='".$code."'";
        $lib=$this->getSQL('db', $ministere, '');
        return $lib[0]['libelle']; 
    }
