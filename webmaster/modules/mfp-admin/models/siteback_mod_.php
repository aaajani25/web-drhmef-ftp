...
class Siteback_mod extends CI_Model {
    ...
    public function do_phototheque($table) {
        ...
        $config['upload_path'] = 'assets/rubriques/' . $table;
        ...
        switch ($table) {
            case "utilisateur":
                ...
                $data = array(
                    ...
                    'pswd' => Password::hash($this->input->post('mdp')),
                    'profil' => $profil,
    ...
    public function mise_a_jour($table, $id) {
        if ($table == '_communique' && $this->input->post('com_sh') == 1) {
            ...
            switch ($table) {
                case 'utilisateur' :
                    $data = array(
                        'pswd' => Password::hash($this->input->post('nouvo_pass1')),
    ...
    public function delete($table, $id) {
        ...
        if ($query > 0) {
            if (!in_array($table, array('access_ef', 'utilisateur', 'categorie_doc'))) {
                ...
                $file_path_wide = $_SERVER['DOCUMENT_ROOT'] . "/mfp/assets/rubriques/" . $table . "/" . $data['image_wide'];
                $file_path_small = $_SERVER['DOCUMENT_ROOT'] . "/mfp/assets/rubriques/" . $table . "/" . $data['image_small'];
